...
<ArticleTimeline
	client:load
	sections={[
		{ id: "vector-field-viz", label: "Interactive Demo", type: "sidequest", parent: "chapter-1" },
...
The geometry must look consistent from all viewing angles. A volumetric line shouldn't look flat from certain directions.

Also, the geometry must adapt to its spatial context, which in this case is the direction and magnitude sampled at a spatial location.
Importing static or pre-animated 3D objects wouldn't allow this kind of adaptivity, which means we must generate the geometry procedurally.  

Finally, it must run smoothly in real time, while looking believable, so we need a minimal rendering procedure.
...
The video below illustrates the construction process:
...
Each dipole creates a field based on its magnetic moment and distance: `B = (3(m·r̂)r̂ - m) / r³`. Combining two dipoles produces the characteristic looping field lines.
...
<details style={{ margin: "16px auto", maxWidth: "800px" }}>
<summary style={{ cursor: "pointer", color: "#888", fontStyle: "italic" }}>MagnetPhysics.ts: Opposite poles attract, like poles repel. Shake to separate when stuck.</summary>
...
Adding an alpha falloff and exporting as .png:

<div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "8px", maxWidth: "600px", margin: "16px auto" }}>
	<div style={{ textAlign: "center" }}>
		<img src="/assets/visualizing-vector-fields-on-ar-glasses/sprites/contraction_preview.png" alt="Contraction frames with alpha" loading="lazy" decoding="async" style={{ width: "100%", borderRadius: "8px" }} />
		<p style={{ fontSize: "0.75em", color: "#888", margin: "4px 0" }}>Contraction</p>
	</div>
	<div style={{ textAlign: "center" }}>
		<img src="/assets/visualizing-vector-fields-on-ar-glasses/sprites/expansion_preview.png" alt="Expansion frames with alpha" loading="lazy" decoding="async" style={{ width: "100%", borderRadius: "8px" }} />
		<p style={{ fontSize: "0.75em", color: "#888", margin: "4px 0" }}>Expansion</p>
	</div>
	<div style={{ textAlign: "center" }}>
		<img src="/assets/visualizing-vector-fields-on-ar-glasses/sprites/circulation_preview.png" alt="Circulation frames with alpha" loading="lazy" decoding="async" style={{ width: "100%", borderRadius: "8px" }} />
		<p style={{ fontSize: "0.75em", color: "#888", margin: "4px 0" }}>Circulation</p>
	</div>
	<div style={{ textAlign: "center" }}>
		<img src="/assets/visualizing-vector-fields-on-ar-glasses/sprites/vortex_preview.png" alt="Vortex frames with alpha" loading="lazy" decoding="async" style={{ width: "100%", borderRadius: "8px" }} />
		<p style={{ fontSize: "0.75em", color: "#888", margin: "4px 0" }}>Vortex</p>
	</div>
	<div style={{ textAlign: "center" }}>
		<img src="/assets/visualizing-vector-fields-on-ar-glasses/sprites/waves_preview.png" alt="Waves frames with alpha" loading="lazy" decoding="async" style={{ width: "100%", borderRadius: "8px" }} />
		<p style={{ fontSize: "0.75em", color: "#888", margin: "4px 0" }}>Waves</p>
	</div>
	<div style={{ textAlign: "center" }}>
		<img src="/assets/visualizing-vector-fields-on-ar-glasses/sprites/magnetic_preview.png" alt="Magnetic frames with alpha" loading="lazy" decoding="async" style={{ width: "100%", borderRadius: "8px" }} />
		<p style={{ fontSize: "0.75em", color: "#888", margin: "4px 0" }}>Magnetic</p>
	</div>
</div>
...
<details style={{ margin: "24px auto", maxWidth: "800px" }}>
<summary style={{ cursor: "pointer", color: "#888", fontStyle: "italic" }}>AnimatedSpriteSheet.js</summary>
...
vec4 samplePreset(int p, vec2 frameUV) {
    if (p == 0) return expansionSheet.sample(frameUV);
    if (p == 1) return contractionSheet.sample(frameUV);
    if (p == 2) return circulationSheet.sample(frameUV);
    if (p == 3) return wavesSheet.sample(frameUV);
    if (p == 4) return vortexSheet.sample(frameUV);
    if (p == 5) return magneticSheet.sample(frameUV);
    return vec4(0.0);
}
...
void main() {
    ...
    vec2 frameUV = vec2(
        float(col) * cellWidth + uv.x * cellWidth,
        mix(1.0 - float(row + 1) * cellHeight, 1.0 - float(row) * cellHeight, uv.y)
    );
